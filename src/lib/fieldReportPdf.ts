...
/**
 * Generate a Daily Field Report PDF from a report object + project.
 * Returns { buffer, filename }.
 */
export async function generateFieldReportPdf(
  report: any,
  project: { name: string },
  ...
  const jobNameText = sanitize(report.project?.name || project.name || "N/A");
  ...
  const filename = `DFR-${report.date}-${project.name?.replace(/\s+/g, "-").slice(0, 30) || "report"}.pdf`;
