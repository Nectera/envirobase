...
export default async function PreAbatementListPage() {
  const language: Language = "en";
  const t = (key: string) => getTranslation(language, key);
  const items = await prisma.preAbatementInspection.findMany({ include: { project: true } });
  ...
  function getPassFailCounts(item: any) {
    const checks = item.checklistItems || {};
    const yes = Object.values(checks).filter((v: any) => v === "yes").length;
    const no = Object.values(checks).filter((v: any) => v === "no").length;
    const total = Object.values(checks).filter((v: any) => v !== "na").length;
    return { yes, no, total };
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-slate-900">{t("preAbatement.title")}</h1>
          <p className="text-sm text-slate-500 mt-0.5">{t("preAbatement.subtitle")}</p>
        </div>
        <Link href="/pre-abatement-inspection/new"
          ...
          <Plus size={16} /> {t("preAbatement.newInspection")}
        </Link>
      </div>
      ...
      {items.length === 0 ? (
        <div className="text-center py-12 bg-white rounded-lg border border-slate-200">
          ...
          <h3 className="font-semibold text-slate-700">{t("preAbatement.noInspections")}</h3>
          <p className="text-sm text-slate-500 mt-1">{t("preAbatement.noInspectionsDescription")}</p>
        </div>
      ) : (
            ...
            <div key={projId} className="bg-white rounded-lg border border-slate-200 overflow-hidden">
              <div className="px-5 py-3 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-slate-400">{group.items.length} inspections</span>
                </div>
                <Link href={`/pre-abatement-inspection/new?projectId=${projId}`}
              ...
              <table className="w-full text-sm">
                <thead className="bg-slate-50/50 border-b border-slate-100">
                  <tr>
                    ...
                    <th className="text-left px-4 py-2 font-medium text-slate-500 text-xs">{t("preAbatement.inspector")}</th>
                    <th className="text-left px-4 py-2 font-medium text-slate-500 text-xs">{t("common.supervisor")}</th>
                    <th className="text-left px-4 py-2 font-medium text-slate-500 text-xs">{t("preAbatement.results")}</th>
                ...
                <tbody className="divide-y divide-slate-50">
                  {group.items.map((item: any) => {
                    const { yes, no, total } = getPassFailCounts(item);
                    const resultColor = no === 0 ? "text-green-700" : no > 0 && yes > 0 ? "text-yellow-700" : "text-red-700";
                    return (
                      <tr key={item.id} className="hover:bg-slate-50">
                        ...
                        <td className="px-4 py-2 text-slate-600">{item.contractorSupervisor || "—"}</td>
